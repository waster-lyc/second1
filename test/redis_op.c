...
int rop_set_hash(redisContext* conn, const char *key,const char *field, const char *value)
{
	int retn = 0;

	redisReply *reply = NULL;

	reply = redisCommand(conn, "HMSET %s %s %s", key, field,value);
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]SET %s %s error %s\n", key, value,conn->errstr);
        retn = -1;
		goto END;
...
int rop_get_hash(redisContext*conn, const char *key,const char *field, char *values)
{
	int retn = 0;
	...
	if (reply == NULL) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]HMGET %s  error %s\n", key, conn->errstr);
        retn = -1;
		goto END;
	}
	strcpy(values,reply->str);
    printf("hmget %s %s succ!\n", key, field);
